...
const TransactionList: React.FC = () => {
    ...
    const loadTransactions = useCallback(async () => {
        try {
            ...
            if (isAdmin()) {
                response = await transactionService.findAllTransactionsWithPagination(page, size);
            } else if (isUser()) {
                response = await transactionService.findAllTransactionsWithPaginationByAuthUser(page, size);
            }
    ...
    const searchTransactions = useCallback(async () => {
        try {
            ...
            if (isAdmin()) {
                response = await transactionService.findAllTransactionsBySearchWithPagination(searchText, page, size);
            } else if (isUser()) {
                response = await transactionService.findAllTransactionsBySearchWithPaginationByAuthUser(searchText, page, size);
            }
    ...
    const transactionViewHandler = async (id: string) => {
        try {
            ...
            if (isAdmin()) {
                response = await transactionService.findTransactionById(id);
            } else if (isUser()) {
                response = await transactionService.findTransactionByIdWithByAuthUser(id);
            }
