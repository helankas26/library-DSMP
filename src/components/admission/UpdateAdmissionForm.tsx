...
const UpdateAdmissionForm: React.FC<{
    admission: Admission;
    setUpdateAdmission: Dispatch<SetStateAction<Admission | undefined>>;
    setToggleUpdate: Dispatch<SetStateAction<boolean>>;
    onRefreshAdmissions: () => Promise<void>
}> = (props) => {
    const {admission, setUpdateAdmission, setToggleUpdate, onRefreshAdmissions} = props;
    const {showError, showAlert} = useSnackbar();

    const [fee, setFee] = useState<number>(admission.fee);
    const [isUpdating, setIsUpdating] = useState<boolean>(false);

    const updateAdmissionHandler = async (event: FormEvent) => {
        event.preventDefault();

        setIsUpdating(true);

        const editedAdmission: Admission = {
            fee: fee
        } as Admission;

        try {
            await admissionService.updateAdmission(admission._id, editedAdmission);
            showAlert("Admission updated successfully!", "success");
            await onRefreshAdmissions();
            setToggleUpdate(false);
            setUpdateAdmission(undefined);
        } catch (error: any) {
            showError(error);
        } finally {
            setIsUpdating(false);
    ...
    return (
        <div className="min-w-full border rounded mb-6">
            <div className="mx-auto max-w-screen-xl p-4 flex flex-col gap-5">
                ...
                <Form className="flex flex-col sm:flex-row sm:items-end gap-5 sm:gap-10" onSubmit={updateAdmissionHandler}>
